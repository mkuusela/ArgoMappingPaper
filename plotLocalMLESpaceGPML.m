...
for iGrid = 1:nGrid
    corrLat800(iGrid) = spaceTimeCovarianceExpGeom(deltaLat,0,0,0,0,0,thetasOpt(iGrid),thetaLatOpt(iGrid),thetaLongOpt(iGrid),1)/(thetasOpt(iGrid) + nuOpt(iGrid)/(nuOpt(iGrid)-2) * sigmaOpt(iGrid).^2);
end
...
for iGrid = 1:nGrid
    corrLon800(iGrid) = spaceTimeCovarianceExpGeom(0,deltaLon(iGrid),0,0,0,0,thetasOpt(iGrid),thetaLatOpt(iGrid),thetaLongOpt(iGrid),1)/(thetasOpt(iGrid) + nuOpt(iGrid)/(nuOpt(iGrid)-2) * sigmaOpt(iGrid).^2);
end
