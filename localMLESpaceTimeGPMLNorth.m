...
thetasInit = thetasInit(:,91:end);
thetaLatInit = S.thetaLatOpt;
thetaLatInit = thetaLatInit(:,91:end);
thetaLongInit = S.thetaLongOpt;
thetaLongInit = thetaLongInit(:,91:end);
thetatInit = S.thetatOpt;
thetatInit = thetatInit(:,91:end);
sigmaInit = S.sigmaOpt;
sigmaInit = sigmaInit(:,91:end);
...
fileName = ['iterIdxLocalMLESpaceTimeGPMLNorth_',num2str(presLevel),'_',num2str(month,'%02d'),'.txt'];
...
save(['./Results/localMLESpaceTimeGPMLNorth_',num2str(presLevel),'_',num2str(month,'%02d'),'_',num2str(startYear),'_',num2str(endYear),'.mat'],'latGrid','longGrid','thetasOpt','thetaLatOpt','thetaLongOpt','thetatOpt','sigmaOpt','nuOpt','nll','nResGrid');
