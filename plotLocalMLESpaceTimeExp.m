...
%surfm(latGrid,longGrid,mask.*(thetasOpt + sigmaOpt.^2));
...
for iGrid = 1:nGrid
    corrLat800(iGrid) = spaceTimeCovarianceExpGeom(deltaLat,0,0,0,0,0,thetasOpt(iGrid),thetaLatOpt(iGrid),thetaLongOpt(iGrid),thetatOpt(iGrid))/(thetasOpt(iGrid) + sigmaOpt(iGrid).^2);
end
...
for iGrid = 1:nGrid
    corrLon800(iGrid) = spaceTimeCovarianceExpGeom(0,deltaLon(iGrid),0,0,0,0,thetasOpt(iGrid),thetaLatOpt(iGrid),thetaLongOpt(iGrid),thetatOpt(iGrid))/(thetasOpt(iGrid) + sigmaOpt(iGrid).^2);
end
...
for iGrid = 1:nGrid
    corrT10(iGrid) = spaceTimeCovarianceExpGeom(0,0,10,0,0,0,thetasOpt(iGrid),thetaLatOpt(iGrid),thetaLongOpt(iGrid),thetatOpt(iGrid))/(thetasOpt(iGrid) + sigmaOpt(iGrid).^2);
end
...
surfm(latGrid,longGrid,mask.*corrT10);
